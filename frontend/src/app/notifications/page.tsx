    ...
    <main className="login-pdf-font min-h-dvh bg-[#FFFEFC] px-3 py-4 text-[#222222]">
      <div className="mx-auto w-full max-w-[380px] pb-8">
        <header className="relative mb-4 pt-1 text-center">
          ...
          <p className="mt-0.5 text-[13px] font-normal text-[#77716E]">업무 소식을 모아봤어요</p>
        </header>
        ...
        <section className="rounded-[16px] border border-[#D8D1CE] bg-white p-3 shadow-[0_2px_10px_rgba(95,73,68,0.08)]">
          {isLoading ? (
            <p className="py-8 text-center text-[13px] font-normal text-[#7B716D]">알림을 불러오는 중입니다.</p>
          ) : null}

          {!isLoading && notifications.length === 0 ? (
            <p className="py-8 text-center text-[13px] font-normal text-[#7B716D]">아직 알림이 없습니다.</p>
          ) : null}
            ...
            {notifications.map((notification) => (
              ...
              >
                ...
                <div className="min-w-0">
                  <h2 className="truncate text-[15px] font-normal text-[#111111]">{notification.task.title}</h2>
                  <p className="mt-1 break-keep text-[13px] font-normal leading-5 text-[#4F4542]">
                    {notification.type === "TASK_ASSIGNED"
                  ...
                  {notification.type === "TASK_COMMENTED" && notification.commentPreview ? (
                    <p className="mt-1 truncate rounded-[10px] bg-[#F7F7F7] px-2 py-1 text-[12px] font-normal text-[#7B716D]">
                      {notification.commentPreview}
                    </p>
                  ) : null}
                  <p className="mt-1 text-[11px] font-normal text-[#9B9592]">{formatNotificationDate(notification.createdAt)}</p>
                </div>
              </button>
            ))}
          </div>
