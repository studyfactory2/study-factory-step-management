...
export function RecentOutputsSection({
  ...
  return (
    <section className="rounded-[18px] border border-[#dce5f2] bg-[linear-gradient(145deg,#ffffff_0%,#f8fbff_62%,#faf8ff_100%)] p-3 shadow-[0_8px_24px_rgba(49,91,140,0.09)] sm:p-4">
      ...
      <div className="mb-2.5 flex items-center justify-between gap-1 sm:gap-2">
        <div className="flex min-w-0 items-center gap-0.5 sm:gap-1 md:gap-1.5">
          {statusOptions.map((option) => {
            ...
            return (
              <button
                className={`h-6 w-[50px] shrink-0 rounded-[7px] border px-0 text-[10px] font-medium leading-none transition ${
                  isSelected
        ...
        {showScopeSelector ? (
          <div className="relative shrink-0" ref={scopeDropdownRef}>
            <button
              ...
              className="flex h-6 w-20 items-center justify-between rounded-[7px] border border-[#D8D1CE] bg-white pl-2 pr-1.5 text-left text-[10px] font-medium leading-none text-[#333333] outline-none"
            ...
            {isScopeOpen && (
              <div className="absolute right-0 top-7 z-30 w-[108px] overflow-hidden rounded-[9px] border border-[#D8D1CE] bg-white py-1 shadow-[0_8px_18px_rgba(95,73,68,0.16)]">
      ...
      <div className="grid grid-cols-[5px_64px_minmax(0,1fr)_52px_52px] gap-1 pb-1.5 pr-0.5 text-center text-[10px] font-normal leading-tight text-[#7B716D] sm:grid-cols-[5px_72px_minmax(0,1fr)_72px_72px] sm:text-[13px] md:grid-cols-[5px_124px_minmax(0,1fr)_82px_82px] md:pr-4 md:text-[14px] lg:grid-cols-[5px_156px_minmax(0,1fr)_96px_96px] lg:text-[15px]">
        <span aria-hidden />
        <span>최초 작성자</span>
        <span>업무 제목</span>
        <span>상태</span>
      ...
      <div className="max-h-[330px] space-y-1.5 overflow-y-auto pr-0.5 sm:max-h-[460px] md:pr-4">
        ...
        {filteredOutputs.map((output) => {
          ...
          const completedTextClassName = isCompleted
            ? "line-through decoration-[#8E8581] decoration-1"
            : "";

          return (
            <button
              className="grid min-h-[56px] w-full grid-cols-[5px_64px_minmax(0,1fr)_52px_52px] items-stretch overflow-hidden rounded-[10px] border border-[#E7E0DD] bg-white text-left shadow-[0_1px_4px_rgba(95,73,68,0.06)] sm:min-h-[68px] sm:grid-cols-[5px_72px_minmax(0,1fr)_72px_72px] md:min-h-[76px] md:grid-cols-[5px_124px_minmax(0,1fr)_82px_82px] lg:grid-cols-[5px_156px_minmax(0,1fr)_96px_96px]"
              key={output.taskId}
              onClick={() => onDetailOpen(output.taskId)}
              type="button"
            >
              <span className={getStatusBarClassName(output.taskStatus)} />
              <span className="flex min-w-0 flex-col items-start justify-center pl-1.5 pr-1 text-left font-normal leading-none sm:pl-3 md:pl-4 md:pr-2">
                <span className="flex max-w-full flex-col items-start md:flex-row md:items-center md:gap-1">
                  <span
                    className={`line-clamp-1 max-w-full break-keep text-[11px] sm:text-[13px] md:max-w-[64px] md:text-[15px] lg:max-w-[76px] lg:text-[16px] ${isCreatorReceiver && !isCompleted ? "text-[#2D70CB]" : "text-[#4F4542]"} ${completedTextClassName}`}
                  >
                    {output.creatorName ?? output.memberName}
                  </span>
                  <span
                    className={`hidden text-[13px] text-[#7B716D] md:inline lg:text-[14px] ${completedTextClassName}`}
                  >
                    -
                  </span>
                  <span
                    className={`mt-0.5 max-w-full whitespace-nowrap text-[8px] tracking-[-0.06em] text-[#7B716D] sm:text-[9px] md:mt-0 md:max-w-none md:text-[10px] lg:text-[11px] ${completedTextClassName}`}
                  >
                    {output.creatorOrganizationName ?? "미지정"}
                  </span>
                </span>
                <span
                  className={`mt-0.5 whitespace-nowrap text-[8px] text-[#9A918D] sm:text-[10px] md:mt-1 md:text-[12px] lg:text-[13px] ${completedTextClassName}`}
                >
              ...
              <span className="flex min-w-0 flex-col justify-center pl-5 pr-1.5 text-left sm:items-start sm:pl-6 sm:pr-3 md:pl-[30%] md:pr-4 lg:pl-[34%] lg:pr-5">
                <span className="flex min-w-0 max-w-full items-center gap-1">
                  <span
                    className={`truncate pt-px text-[12px] font-normal leading-[1.25] text-[#222222] sm:text-[14px] sm:leading-[1.3] md:text-[16px] lg:text-[17px] ${completedTextClassName}`}
                  >
                    {output.taskTitle}
                  </span>
                ...
                {output.oneLineComment && (
                  <span
                    className={`mt-1 flex min-w-0 max-w-full items-center gap-1 text-[9px] font-normal leading-none text-[#9A918D] sm:text-[10px] md:text-[12px] lg:text-[13px] ${completedTextClassName}`}
                  >
                    <MessageCircle
                      aria-hidden
                      className="h-2.5 w-2.5 shrink-0 text-[#9A918D]"
                    />
              ...
              <span className="flex items-center justify-center px-0">
                <span
                  className={`flex h-[18px] w-[50px] items-center justify-center gap-0.5 rounded-full text-[9px] font-normal leading-none sm:h-[22px] sm:w-[62px] sm:text-[11px] md:h-[24px] md:w-[76px] md:text-[13px] lg:w-[84px] lg:text-[14px] ${getStatusBadgeClassName(output.taskStatus)}`}
                >
                  {!isCompleted && direction === "TO_CREATOR" && (
                    <span aria-hidden>←</span>
                  )}
                  <span>{getStatusLabel(output.taskStatus)}</span>
                  {!isCompleted && direction === "TO_MEMBER" && (
                    <span aria-hidden>→</span>
                  )}
                  {isCompleted && (
                    <Check aria-hidden className="h-2.5 w-2.5 stroke-[2.5]" />
                  )}
                </span>
              </span>
              <span className="flex min-w-0 items-center justify-center px-0 text-center text-[11px] font-normal leading-[1.25] sm:text-[12px] sm:leading-[1.3] md:text-[14px] lg:text-[15px]">
                <span
                  className={`line-clamp-2 break-keep pt-px ${isMemberReceiver && !isCompleted ? "text-[#2D70CB]" : "text-[#4F4542]"} ${completedTextClassName}`}
                >
                  {output.memberName}
                </span>
              </span>
...
function filterOutputsByScope(
  ...
  if (scope === "MINE") {
    return outputs.filter(
      (output) =>
        output.memberId === currentMemberId ||
        output.creatorId === currentMemberId,
    );
  }
...
function filterOutputsByStatus(
  outputs: AdminDashboardRecentOutput[],
  selectedStatuses: TaskStatus[],
) {
